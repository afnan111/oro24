...
const DashboardHome = () => {
  ...
  const handleAssignInspection = async () => {
    try {
      const selectedItems = items.filter((item) =>
        selectedUnits.includes(item.ProjectDeliveryID)
      );
      ...
      if (selectedItems.length > 0) {
        ...
        const response = await fetch(
          "https://oro24world.com/api/HandOverProjectDelivery/add-inspection-space",
          {
            method: "POST",
            headers: {
              "Content-Type": "application/json",
  ...
  const filteredItems = items.filter((unit) => {
    const floorMatch =
      `Floor 1`.toLowerCase().trim() === unit.FloorNo.toLowerCase().trim() ||
      `Floor 2`.toLowerCase().trim() === unit.FloorNo.toLowerCase().trim();
    const apartmentTypeMatch =
      selectedApartmentType === "All" ||
      unit.ApartmentType === selectedApartmentType;
  ...
  return (
    <div className="dashboard-home">
      ...
      <div className="block-tabs">
        {["BLOCK 1", "BLOCK 2", "BLOCK 3", "BLOCK 4", "BLOCK 5"].map(
          (block) => (
            <button
              key={block}
      ...
      <div style={{ display: "flex", justifyContent: "space-between" }}>
        <div className="apartment-filter">
          ...
          >
            <h1 style={{ marginRight: "42px", backgroundColor: "transparent" }}>
              Floor 1
            </h1>
          </button>
          <button
            className={`filter-button ${
              selectedApartmentType === "All" ? "active" : ""
            }`}
            onClick={() => setSelectedApartmentType("All")}
          >
            All
          ...
          <button
            className={`filter-button ${
              selectedApartmentType === "Studio" ? "active" : ""
            }`}
          ...
          <button
            className={`filter-button ${
              selectedApartmentType === "1 Bedroom" ? "active" : ""
            }`}
          ...
          <button
            className={`filter-button ${
              selectedApartmentType === "2 Bedroom" ? "active" : ""
            }`}
          ...
          <button
            className={`filter-button ${
              selectedApartmentType === "3 Bedroom" ? "active" : ""
            }`}
            ...
            <div className="units-grid">
              ...
              {filteredItems.length > 0 ? (
                filteredItems
                  .filter((unit) => {
                    return (
                      `Floor ${floor}`.toLowerCase().trim() ===
                      unit.FloorNo.toLowerCase().trim()
                    );
                  })
                  .map((unit) => (
                    <div
                      ...
                      className={`unit-box ${
                        selectedUnits.includes(unit.ProjectDeliveryID)
                          ? "selected"
                          : ""
                      }`}
                    ...
                    >
                      <span className="apartment-type">
                        {unit.ApartmentType === "Studio"
                          ? "S"
                          : unit.ApartmentType === "1 Bedroom"
                          ? "1B"
                          : unit.ApartmentType === "2 Bedroom"
                          ? "2B"
                          : unit.ApartmentType === "3 Bedroom"
                          ? "3B"
                          : unit.ApartmentType}
                      </span>{" "}
                      {unit.UnitNo} -{" "}
      ...
      <div className="floating-footer">
        <h4 style={{ marginLeft: "-252px", paddingRight: "12px" }}>
          You have Selected {selectedUnits.length} Units
        </h4>
